...
aggregated_difficulty <- ddply(data_difficulty,
                               ...
                               summarise,
                               mean_value = mean(difficulty)
                               
)

aggregated_data <-aggregated_difficulty
fileName <-"./routputs/comparisons/difficulty/techniqueOrderCameraComparisonsDifficulty"
...
for(i in 2:ncol(result_df)) {# for-loop over columns
  ...
  if (k<=ncol(result_df)) {
    for (j in k:ncol(result_df)) {
      ...
      barChart(datatoprint, analysisData$name, nbTechs = 1, ymin = -5, ymax = 5, mycolor = "steelblue3", "", "")
