...
fileName <-"./routputs/comparisons/difficulty/orderCameraComparisonsDifficulty"
...
for(i in 2:ncol(result_df)) {# for-loop over columns
  ...
  if (k<=ncol(result_df)) {
    for (j in k:ncol(result_df)) {
      ...
      barChart(datatoprint, analysisData$name, nbTechs = 1, ymin = -3, ymax = 3, mycolor = "steelblue3", "", "")
