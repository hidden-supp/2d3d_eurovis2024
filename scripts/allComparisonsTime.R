...
fileName <-"./routputs/comparisons/time/techniqueOrderCameraComparisonsTime"
...
for(i in 2:ncol(result_df)) {# for-loop over columns
  ...
  if (k<=ncol(result_df)) {
    for (j in k:ncol(result_df)) {
      ...
      barChart(datatoprint, analysisData$name, nbTechs = 1, ymin = -20, ymax = 20, mycolor = "steelblue3", "", "")
