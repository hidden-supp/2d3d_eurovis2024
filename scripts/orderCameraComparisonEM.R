...
fileName <-"./routputs/comparisons/errormeasure/orderCameraComparisonsEM"

# Create three arrays
array1 <- c("CF", "Coupled", "FF")
array2 <- c("RF", "ZF")
...
for(i in 1:nrow(combinations_df)) {       # for-loop over rows
  ...
  elements<-subset(elements, renderingOrder==combination$Var1)
  elements<-subset(elements, cameraTransition==combination$Var2)
  elements$factors <- paste(elements$renderingOrder, elements$cameraTransition, sep = "_")
