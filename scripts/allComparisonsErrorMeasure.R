...
aggregated_error <- ddply(data_withouttraining,
                          ...
                          summarise,
                          mean_value = mean(firstErrorMeasure)
                          
)

aggregated_data <- aggregated_error
fileName <-"./routputs/comparisons/errormeasure/techniqueOrderCameraComparisonsEM"

# Create three arrays
array1 <- c("Dragalong", "Animated")
array2 <- c("CF", "Coupled", "FF")
array3 <- c("RF", "ZF")

# Create a data frame with all combinations
combinations_df <- expand.grid(array1, array2, array3)
...
for(i in 1:nrow(combinations_df)) {       # for-loop over rows
  ...
  elements<-subset(elements, renderingOrder==combination$Var2)
  elements<-subset(elements, cameraTransition==combination$Var3)
  elements$factors <- paste(elements$technique, elements$cameraTransition, elements$renderingOrder, sep = "_")
  participants_df<-data.frame(participant)
  participants_df$factors<-paste(combination$Var1, combination$Var2, combination$Var3, sep = "_")
...
for(i in 2:ncol(result_df)) {# for-loop over columns
  ...
  if (k<=ncol(result_df)) {
    for (j in k:ncol(result_df)) {
      ...
      barChart(datatoprint, analysisData$name, nbTechs = 1, ymin = -0.25, ymax = 0.25, mycolor = "steelblue3", "", "")
